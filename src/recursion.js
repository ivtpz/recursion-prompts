...
var palindrome = function(string) {
  ...
  if (string[0] === ' ') return palindrome(string.slice(1))
  if (string[string.length - 1] === ' ') return palindrome(string.slice(0, string.length - 1))
...
var tagCount = function(tag, node) {
  node = node || document;
  let addToCount = node.nodeName.toLowerCase() === tag ? 1 : 0
  if (!node.children) {
    return addToCount
  }
  let childrenArray = [...node.children]
  return addToCount + childrenArray.reduce((accum, child) => accum + tagCount(tag, child), 0) 
};
...
var binarySearch = function(array, target, min = 0, max = array.length-1) {
  ...
  let curr = Math.floor(min + (max - min)/2);
  if (array[curr] === target) return curr;
  return array[curr] > target ? 
    binarySearch(array, target, min, curr - 1) :
    binarySearch(array, target, curr + 1, max); 
};
